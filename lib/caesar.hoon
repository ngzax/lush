...
::
:: Perform all possible unshifts until back to original
::
++  unshift-all
  |=  message=tape
  ^-  (list tape)
  =/  next-message  (unshift message 1)
  =/  start-letter  (trim 1 message)
  =/  result        *(list tape)
  |-
  ?:  =(start-letter (trim 1 next-message))
    result
  %=  $
    result        (weld result ~[next-message])
    next-message  (unshift next-message 1)
  ==
